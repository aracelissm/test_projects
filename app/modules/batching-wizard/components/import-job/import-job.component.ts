...
export class ImportJobComponent extends BaseWizardStepComponent {
    ...
    jobCopyDownForm: UntypedFormGroup = new UntypedFormGroup({
        ...
        dupJobs: new UntypedFormControl('')
    });
            ...
            next: (res) => {
                if (res.success)
                {
                    ...
                    this.jobCopyDownForm.patchValue({
                        qtyX: res.data?.batchOptionsNumDups,
                        dupJobs: res.data?.batchDupJobs
                    });
                }
                else
                {
            ...
            this.dataSource = new CustomStore({
                ...
                load: async (loadOptions: any) => {
                    ...
                    return await firstValueFrom(
                        this.jobItemService.getTrussJobItems({ ...subQuery, jobKey })
                    )
                        .then((res) => {
                            if (res && res.success) {
    ...
    copyDown() {
        ...
        const { jobKey, copyToJob, copyToPath, qtyX, dupJobs } = this.jobCopyDownForm.value;

        // TODO: You should pass along the dupJobsField From front end.
    ...
    loadPicFile() {
        this.isComplete = false;
        this.resetCompleteStatus();
        this.importJobService.loadPicFile(this.jobKey).subscribe({
            next: (res) => {
                if (res && res.success && res.data) {
                    this.isComplete = true;
                    this.dataGrid.instance.refresh();
